...
def test_get_for_date_normal_workday(date_dim_service):
    test_date = datetime(2024, 4, 5, 15, 0, 0)  # 任意正常工作日日期时间
    ...
    expected_week_identifier = test_date.strftime("%V")
    expected_year_quarter = test_date.strftime("%YQ%q")
    ...
    expected_shift = (
        "早" if test_date.hour < 17 else "夜" if test_date.hour < 8 else "中"
    )
    expected_holiday_name = "清明节"
    expected_date_type = "节假日"

    result = date_dim_service.get_for_date(test_date)
